...
export const incrementRightmostNumber = (str, iteration) => {
	...
	if (match) {
		...
		const incrementedNumber =
			Number.parseInt(rightmostNumber, 10) + Number.parseInt(iteration, 10);
		const startPosition = match.index;
		const endPosition = startPosition + rightmostNumber.length;
		return (
			str.slice(0, startPosition) + incrementedNumber + str.slice(endPosition)
		);
	}
...
export const addAnotherTool = (e) => {
	...
	const autoincrementBarcode = document.querySelector(
		"#autoincrement-barcode-toggle",
	);
	const autoincrementSerialNumber = document.querySelector(
		"#autoincrement-serialNumber-toggle",
	);
	const autoincrementToolID = document.querySelector(
		"#autoincrement-toolID-toggle",
	);
	const autoincrementCheckboxes = document.querySelectorAll(
		'[id^="autoincrement-"]',
	);
	...
	additionalToolElement.classList.add(
		"grid",
		"grid-rows-1",
		"grid-cols-3",
		"gap-2",
	...
	if (autoincrementBarcode.checked) {
		if (endsWithNumber(baseBarcode) === false || baseBarcode === "") {
			...
			barcodeElement.value = incrementRightmostNumber(
				baseBarcode,
				entriesCount,
			);
	...
	if (autoincrementSerialNumber.checked) {
		if (endsWithNumber(baseSerialNumber) === false || baseSerialNumber === "") {
			...
			const serialNumberElement = document.querySelector(
				`#serialNumber_${entriesCount}`,
			);
			serialNumberElement.value = incrementRightmostNumber(
				baseSerialNumber,
				entriesCount,
			);
	...
	if (entriesCount === 1) {
		for (const element of autoincrementCheckboxes) {
