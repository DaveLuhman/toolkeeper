...
controller.getUniqueTool() = async (req, res) => {
    ...
    const tool = _id ? await Tool.findById(_id) : await Tool.findOne({ serialNumber: serialNumber })

    // let tool
    // if(!_id && !serialNumber) { return res.status(400).send('No unique value provided')}
    // if(_id) {
    //     tool = await Tool.findById(_id)
    // }
    // else if(serialNumber) {
    //     tool = await Tool.findOne({ serialNumber: serialNumber })
    // }
